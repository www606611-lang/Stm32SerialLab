...
下次启动会恢复窗口位置和大小、主题、上次串口、波特率、接收/发送格式、行尾、自动滚动、Demo 开关、当前工作页、Scope 时间窗/增益/Y 轴模式，以及各遥测变量的显示状态。串口连接、Pause/Hold、历史日志、实时计数和遥测样本属于一次运行的现场，不会自动恢复，避免启动时误连硬件或混入旧数据。
